...
function buildCamp() {
  ...
  game.bossDefeated = false;
  game.houses = [
    { id: "camp-merchant", x: 205, y: 210, w: 150, h: 110, label: "Basic Merchant", loot: "none" },
    { id: "camp-blacksmith", x: 520, y: 205, w: 168, h: 116, label: "Locked Smithy", loot: "none" },
  ];
  game.campObjects = [
    { id: "campfire", type: "campfire", x: 330, y: 430, w: 42, h: 42, label: "Old Campfire" },
    { id: "sealed-door", type: "sealedDoor", x: 820, y: 245, w: 76, h: 96, label: "Sealed Stone Door" },
    { id: "dungeon-entrance", type: "dungeonEntrance", x: 790, y: 1040, w: 120, h: 90, label: "Dungeon Entrance" },
    { id: "dummy", type: "dummy", x: 1040, y: 470, w: 34, h: 58, hp: 999, maxHp: 999, label: "Training Dummy" },
  ];
  game.player.x = 330;
  game.player.y = 520;
  spawnNpc(260, 365, "Pip the Pack-Seller", "shop");
  spawnNpc(610, 365, "Anvil Auntie", "smithLocked");
  spawnNpc(430, 455, "Old Campfire Keeper", "campGuide");
  closeMenu();
  updateCamera();
}
...
function updateCamera() {
  game.camera.x = clamp(game.player.x - canvas.width / 2, 0, MAP_W - canvas.width);
  game.camera.y = clamp(game.player.y - canvas.height / 2, 0, MAP_H - canvas.height);
}
...
function drawCamp() {
  ctx.fillStyle = "#34543a";
  ctx.fillRect(0, 0, MAP_W, MAP_H);
  ctx.fillStyle = "#456b49";
  for (let y = 0; y < MAP_H / TILE; y++) {
    for (let x = 0; x < MAP_W / TILE; x++) {
      if ((x + y * 2) % 6 === 0) ctx.fillRect(x * TILE, y * TILE, TILE, TILE);
    }
  }
  ctx.fillStyle = "#7b6134";
  ctx.fillRect(270, 400, 650, 84);
  ctx.fillRect(780, 400, 86, 710);
  ctx.fillRect(170, 330, 1040, 72);
  drawGroundDetails();
  ...
  ctx.fillRect(180, 115, 520, 34);
  ctx.fillStyle = "#f4efd7";
  ctx.font = "20px Courier New";
  ctx.fillText("Adventurer's Camp - safe for now", 196, 139);
}
...
function drawCampObject(object) {
  ...
  drawSmallLabel(object.x, object.y - 58, object.label);
}
...
function refreshUi() {
  ...
  ui.worldName.textContent = game.backrooms ? "Backrooms" : game.inCamp ? "Adventurer's Camp" : world.name;
